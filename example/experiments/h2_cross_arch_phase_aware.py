"""
H2 Cross-Architecture Baseline — Phase-Aware vs Standard MTT
=============================================================

Extends h1_cross_arch_baseline.py to compare two distillation methods
across the full expert -> student architecture matrix:
...
Evaluation protocol (same as h1_cross_arch_baseline.py / experiment_matrix.py):
  - real and synthetic training both use early stopping on the real
    validation set (eval_max_epochs=50, early_stop_patience=10)
...
Output
------
  results/cross_arch_phase_aware.csv

Run
---
  python -m example.experiments.h2_cross_arch_phase_aware
"""
...
DISTILL_CONFIG = {
    ...
    # matching h1_cross_arch_baseline.py / experiment_matrix.py.
...
CSV_PATH    = RESULTS_DIR / "cross_arch_phase_aware.csv"
...
def train_expert(model_name, data, cfg, device, seed):
    ...
    # RNG state right after expert training — this is the same point at which
    # h1_cross_arch_baseline.py's distill_synthetic() proceeds to initialize
    # the synthetic sequence. Callers restore this state before each
    # distillation call so synthetic_init matches h1 (for param_mtt) and is
    # identical between param_mtt and phase_aware_mtt (isolating the matching
    # loss as the only difference between the two methods).
...
def distill_param_mtt(model_name, recorder, data, cfg, device, seed):
    ...
    (see train_expert) before calling this, so synthetic_init matches
    h1_cross_arch_baseline.py's distill_synthetic for the same expert/seed.
    """
    sl = data["seq_len"]
    pl = data["pred_len"]
    nf = data["in_features"]
...
def distill_phase_aware_mtt(model_name, recorder, data, cfg, device, seed, phase_boundary):
    ...
    Caller must restore the RNG state captured right after expert training
    (see train_expert) before calling this, so synthetic_init is identical
    to distill_param_mtt's — the matching loss is the only difference
    between the two methods' results.
    """
    sl = data["seq_len"]
    pl = data["pred_len"]
    nf = data["in_features"]
...
# =============================================================================
# EVALUATION (same protocol as h1_cross_arch_baseline.py)
# =============================================================================
...
def run(device, cfg):
    ...
    print("=" * 80)
    print("H2 CROSS-ARCHITECTURE BASELINE — PHASE-AWARE vs STANDARD MTT")
    print("=" * 80)
    ...
    for dataset_name in ACTIVE_DATASETS:
        ...
        for expert_name in ACTIVE_MODELS:
            ...
            for seed in seeds:
                    ...
                    for student_name in ACTIVE_MODELS:
                        for method in methods_needed:
                            if not row_exists(dataset_name, expert_name, student_name, seed, method):
                                append_csv({
                                    "experiment":     "cross_arch_phase_aware",
                ...
                # Restore RNG state to right after expert training (same point
                # h1's distill_synthetic reaches before initializing the
                # synthetic sequence) so synthetic_init matches h1 for the
                # same expert/seed.
