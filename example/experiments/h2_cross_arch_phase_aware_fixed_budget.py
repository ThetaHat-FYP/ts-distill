"""
H2 Cross-Architecture Baseline — Phase-Aware vs Standard MTT (Fixed-Budget Evaluation)
=======================================================================================

Variant of h2_cross_arch_phase_aware.py with NO early stopping during
...
and no patience-based stopping — matching the evaluation approach in
h1_cross_arch_baseline_fixed_budget.py.
...
sharing between methods) is identical to h2_cross_arch_phase_aware.py.
...
Evaluation protocol (fixed-budget, no early stopping):
  - real and synthetic training both run for exactly eval_max_epochs
    epochs, no val_loader / patience.
...
Output
------
  results/cross_arch_phase_aware_fixed.csv

Run
---
  python -m example.experiments.h2_cross_arch_phase_aware_fixed_budget
"""
...
ACTIVE_DATASETS = ["ETTm1"]
ACTIVE_MODELS   = ["DLinear","MLP", "CNN"]

MULTI_SEED_DATASETS = {"ETTm1"}
...
DISTILL_CONFIG = {
    ...
    # patience), matching h1_cross_arch_baseline_fixed_budget.py.
    "eval_max_epochs":     300,
    "eval_lr":             0.001,
    "eval_batch_size":     32,
}
...
CSV_PATH    = RESULTS_DIR / "cross_arch_phase_aware_fixed.csv"
...
# =============================================================================
# EVALUATION (fixed budget — no early stopping, matching
# h1_cross_arch_baseline_fixed_budget.py)
# =============================================================================
...
def eval_on_synthetic(student_name, synthetic_seq, data, cfg, device, seed):
    """Train student on synthetic windows for a FIXED budget (no early
    stopping); returns test MSE.
...
def run(device, cfg):
    ...
    print("=" * 80)
    print("H2 CROSS-ARCHITECTURE BASELINE — PHASE-AWARE vs STANDARD MTT (FIXED BUDGET)")
    print("=" * 80)
                    ...
                    for student_name in ACTIVE_MODELS:
                        for method in methods_needed:
                            if not row_exists(dataset_name, expert_name, student_name, seed, method):
                                append_csv({
                                    "experiment":     "cross_arch_phase_aware_fixed",
