"""
Uncertainty-Based Initializer — Convergence Curve
====================================================
Loads the steps-vs-MSE CSVs produced by experiment_matrix.py's convergence
sweep (--mode convergence, ACTIVE_INITIALIZER='uncertainty'), computes the
Area Under the Curve (AUC) per (dataset, model) pair, and plots the
convergence curve(s). A smaller AUC means the model spent less time at
higher error, i.e. faster convergence.

Run from the project root, after generating the CSVs via:
    python -m example.experiments.experiment_matrix --mode convergence
(with ACTIVE_INITIALIZER = 'uncertainty' in experiment_matrix.py)

    python -m ts_distill.distillation_core.initializer.convergence_speed.uncertainty_curve
"""
